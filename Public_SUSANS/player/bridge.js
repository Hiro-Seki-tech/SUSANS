...
(async function main() {
    ...
    const myExportZipBtn = document.getElementById('my-export-zip');
    const susansSaveExportBtn = document.getElementById('susans-save-export');
    const susansSaveImportBtn = document.getElementById('susans-save-import');
    const susansSaveImportFile = document.getElementById('susans-save-import-file');
    const susansSaveClearBtn = document.getElementById('susans-save-clear');
    const myExitBtn      = document.getElementById('my-exit');
    const myTitleReturnBtn = document.getElementById('my-title-return');
    const myQbPanelBtn   = document.getElementById('my-qb-panel');
    ...
    async function openDropped(file, append) {
        if (!document.body.classList.contains('my-susans-mode')) document.body.classList.remove('panel-hidden');   // MY SUSANSでは専用ボタンでのみ管理画面を開く
    ...
    // ---- Dante98 Reviver v0.6: dedicated maintenance controls ----
    // ZIP writer intentionally uses the ZIP "stored" method (no compression).  SUSANS is small enough
    // that this keeps the implementation self-contained and, more importantly, avoids changing the
    // bytes of Dante98 data files.  Paths are written as their original 8-bit names (UTF-8 flag off),
    // matching QuuBee's latin1/raw-byte representation for DOS archive names.
    let myCurrentGame = null;
    function myEditedZipName() {
        // Keep the original ZIP filename so it can be placed back into games/ without renaming.
        const original = myCurrentGame && typeof myCurrentGame.file === 'string' ? myCurrentGame.file : 'DANTE98.zip';
        return original.replace(/[\\/:*?"<>|]+/g, '_');
    }

    function mySetStatus(text, kind = '') {
        if (!myStatusEl) return;
        myStatusEl.textContent = text || '';
        myStatusEl.classList.remove('ok', 'err');
        if (kind) myStatusEl.classList.add(kind);
    }
    ...
    function dosDateTime(date) {
        ...
        let year = Math.max(1980, Math.min(2107, d.getFullYear()));
    ...
    function makeStoredZip(files) {
        ...
        for (const f of files) {
            const name = rawNameBytes(f.name);
        ...
        const centralBlob = joinBytes(centrals);
        const end = joinBytes([
    ...
    async function exportMySusansZip() {
        if (!myExportZipBtn || myExportZipBtn.disabled) return;
        myExportZipBtn.disabled = true;
        try {
            mySetStatus('編集中のファイルを確認しています…');
            // Bring any last Dante98 writes into QuuBee's view before reading /run directly.
            await syncRunDir();
            const scan = await emu.scanRun();
            const files = [];
            for (let i = 0; i < scan.length; i++) {
                const f = scan[i];
                mySetStatus(`ZIPを作成中… ${i + 1}/${scan.length}  ${sjisName(f.name)}`);
                const data = await emu.readRun(f.name);
                if (data == null) throw new Error(`読み出せません: ${sjisName(f.name)}`);
                files.push({ name: f.name, data, mtime: f.mtimeMs ? new Date(f.mtimeMs) : new Date() });
            }
            if (!files.length) throw new Error('/run/ に書き出せるファイルがありません');
            const blob = makeStoredZip(files);
            const a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            const outName = myEditedZipName();
            a.download = outName;
            document.body.appendChild(a);
            a.click();
            a.remove();
            setTimeout(() => URL.revokeObjectURL(a.href), 1500);
            mySetStatus(`✓ ${outName} を書き出しました（${files.length}ファイル）。元のゲームZIPは変更していません。`, 'ok');
        } catch (e) {
            console.error('MY SUSANS ZIP export failed:', e);
            mySetStatus(`ZIP書き出し失敗: ${e.message}`, 'err');
        } finally {
            myExportZipBtn.disabled = false;
        }
    }

    async function exitMySusans() {
        const ok = confirm('Dante98 Reviverを終了します。\n\n編集済みZIPを書き出していない変更やゲーム内セーブは失われます。\n元のゲームZIPは変更されません。\n\n終了してよいですか？');
        if (!ok) return;
        if (myExitBtn) myExitBtn.disabled = true;
        try {
            mySetStatus('Dante98を停止し、作業領域を破棄しています…');
            await closeBundle();
            mySetStatus('終了しました。ローカルサーバーも終了します…', 'ok');
            try {
                await fetch('/__my_dante98_shutdown__', { method: 'GET', cache: 'no-store' });
            } catch (_) {}
            setTimeout(() => {
                document.body.innerHTML = '<div style="font-family:sans-serif;background:#111;color:#eee;min-height:100vh;display:flex;align-items:center;justify-content:center;text-align:center"><div><h1>Dante98 Reviver</h1><p>終了しました。</p><p style="color:#aaa">このタブを閉じてください。</p></div></div>';
            }, 250);
            // Browsers normally refuse window.close() for tabs opened outside script; try harmlessly,
            // then leave the clear instruction above when the tab cannot be closed automatically.
            try { window.close(); } catch (_) {}
        } catch (e) {
            console.error('MY SUSANS exit failed:', e);
            mySetStatus(`終了処理でエラー: ${e.message}`, 'err');
            if (myExitBtn) myExitBtn.disabled = false;
        }
    }

    function toggleMyQbPanel() {
        const hidden = !document.body.classList.contains('panel-hidden');
        document.body.classList.toggle('panel-hidden', hidden);
        if (myQbPanelBtn) {
            myQbPanelBtn.setAttribute('aria-pressed', hidden ? 'false' : 'true');
            myQbPanelBtn.textContent = hidden ? '⚙ QB管理画面' : '⚙ QB管理画面を閉じる';
        }
        setTimeout(() => fitCanvas(offscreen.width || 640, offscreen.height || 400), 0);
    }

    if (myExportZipBtn) myExportZipBtn.addEventListener('click', exportMySusansZip);
    if (myExitBtn)      myExitBtn.addEventListener('click', exitMySusans);
    if (myQbPanelBtn)   myQbPanelBtn.addEventListener('click', toggleMyQbPanel);

    // ---- SUSANS public save persistence test v0.3 ---------------------------------
    // The original game continues to save into Dante98's own save files directly under /run/.
    // This layer only mirrors those four files to IndexedDB and restores them before launch.
    const SUSANS_SAVE_PATHS = [
        'SAVEDATA.1', 'SAVEDATA.2', 'SAVEDATA.3', 'SAVEDATA.4'
    ];
    const SUSANS_DB_NAME = 'plip_susans_public_save_v2';
    const SUSANS_DB_STORE = 'files';
    let susansSavePollTimer = null;
    let susansLastSaveSig = new Map();
    let susansSaveBusy = false;

    function openSusansSaveDb() {
        return new Promise((resolve, reject) => {
            const req = indexedDB.open(SUSANS_DB_NAME, 1);
            req.onupgradeneeded = () => {
                const db = req.result;
                if (!db.objectStoreNames.contains(SUSANS_DB_STORE)) db.createObjectStore(SUSANS_DB_STORE);
            };
    ...
    async function susansDbPut(path, bytes) {
        const db = await openSusansSaveDb();
        try {
            await new Promise((resolve, reject) => {
                const tx = db.transaction(SUSANS_DB_STORE, 'readwrite');
                tx.objectStore(SUSANS_DB_STORE).put(bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes), path);
    ...
    async function susansDbGet(path) {
        const db = await openSusansSaveDb();
        try {
            return await new Promise((resolve, reject) => {
                const tx = db.transaction(SUSANS_DB_STORE, 'readonly');
                const req = tx.objectStore(SUSANS_DB_STORE).get(path);
                req.onsuccess = () => resolve(req.result == null ? null : new Uint8Array(req.result));
                req.onerror = () => reject(req.error || new Error('IndexedDB read failed'));
            });
        } finally { db.close(); }
    }

    async function clearSusansDb() {
        const db = await openSusansSaveDb();
        try {
            await new Promise((resolve, reject) => {
                const tx = db.transaction(SUSANS_DB_STORE, 'readwrite');
                tx.objectStore(SUSANS_DB_STORE).clear();
                tx.oncomplete = () => resolve();
    ...
    function bytesSignature(bytes) {
        ...
        return bytes.length + ':' + crc32(bytes).toString(16).padStart(8, '0');
    }

    async function restoreSusansBrowserSave() {
        let restored = 0;
        for (const path of SUSANS_SAVE_PATHS) {
            const bytes = await susansDbGet(path);
            if (!bytes) continue;
            await emu.writeRun(path, bytes);
            susansLastSaveSig.set(path, bytesSignature(bytes));
            restored++;
        }
        if (restored) {
            await syncRunDir();
            mySetStatus(`✓ ブラウザ保存からセーブデータを復元しました（${restored}/4）。`, 'ok');
        } else {
            // Establish the ZIP's initial save files as baseline, but do not persist them yet.
            for (const path of SUSANS_SAVE_PATHS) {
                const bytes = await emu.readRun(path);
                if (bytes) susansLastSaveSig.set(path, bytesSignature(bytes));
            }
            mySetStatus('新しいセーブで開始します。ゲーム内でセーブするとブラウザへ自動保存されます。', 'ok');
        }
        return restored;
    }

    async function persistSusansSaves(force = false) {
        if (susansSaveBusy) return 0;
        susansSaveBusy = true;
        const changedPaths = [];
        try {
            for (const path of SUSANS_SAVE_PATHS) {
                const bytes = await emu.readRun(path);
                if (!bytes) continue;
                const sig = bytesSignature(bytes);
                if (!force && susansLastSaveSig.get(path) === sig) continue;
                await susansDbPut(path, bytes);
                susansLastSaveSig.set(path, sig);
                changedPaths.push(path);
            }
            if (changedPaths.length) {
                mySetStatus(`✓ セーブ変更を検出：${changedPaths.join(' / ')} → ブラウザへ自動保存しました。`, 'ok');
            }
            return changedPaths.length;
        } catch (e) {
            console.error('SUSANS browser save failed:', e);
            mySetStatus(`ブラウザ保存に失敗しました: ${e.message}`, 'err');
            return 0;
        } finally { susansSaveBusy = false; }
    }

    function startSusansSavePolling() {
        if (susansSavePollTimer) clearInterval(susansSavePollTimer);
        susansSavePollTimer = setInterval(() => { persistSusansSaves(false); }, 1500);
    }

    async function exportSusansSaveBackup() {
        if (!susansSaveExportBtn || susansSaveExportBtn.disabled) return;
        susansSaveExportBtn.disabled = true;
        try {
            // Capture the latest game writes before making the portable backup.
            await persistSusansSaves(true);
            const files = [];
            for (const path of SUSANS_SAVE_PATHS) {
                let bytes = await emu.readRun(path);
                if (!bytes) bytes = await susansDbGet(path);
                if (!bytes) throw new Error(`${path} が見つかりません`);
                files.push({ name: path, data: bytes, mtime: new Date() });
            }
            const blob = makeStoredZip(files);
            const a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            a.download = 'SUSANS_Save.zip';
            document.body.appendChild(a);
            a.click();
            a.remove();
            setTimeout(() => URL.revokeObjectURL(a.href), 1500);
            mySetStatus('✓ 4つのセーブスロットを SUSANS_Save.zip にまとめて書き出しました。', 'ok');
        } catch (e) {
            console.error('SUSANS save backup export failed:', e);
            mySetStatus(`セーブデータの書き出しに失敗しました: ${e.message}`, 'err');
        } finally {
            susansSaveExportBtn.disabled = false;
        }
    }

    function basenameUpper(name) {
        return String(name || '').replace(/\\/g, '/').split('/').pop().toUpperCase();
    }

    async function importSusansSaveBackup(file) {
        if (!file) return;
        try {
            mySetStatus('セーブデータを読み込んでいます…');
            const bytes = new Uint8Array(await file.arrayBuffer());
            const entries = await qbArchive.parseZip(bytes);
            const byName = new Map();
            for (const ent of entries || []) {
                if (!ent || !ent.data) continue;
                byName.set(basenameUpper(ent.name), new Uint8Array(ent.data));
            }
            const missing = [];
            const saves = [];
            for (const path of SUSANS_SAVE_PATHS) {
                const data = byName.get(path.toUpperCase());
                if (!data) missing.push(path);
                else saves.push({ path, data });
            }
            if (missing.length) {
                throw new Error(`バックアップ内に ${missing.join(', ')} がありません`);
            }

            // Save in both places: persistent browser storage for future launches,
            // and /run so the currently loaded game can use it immediately.
            for (const { path, data } of saves) {
                await susansDbPut(path, data);
                await emu.writeRun(path, data);
                susansLastSaveSig.set(path, bytesSignature(data));
            }
            await syncRunDir();
            mySetStatus('✓ SUSANS_Save.zip を読み込み、4つのセーブスロットを復元しました。ゲーム内の「ロード」から続けられます。', 'ok');
        } catch (e) {
            console.error('SUSANS save backup import failed:', e);
            mySetStatus(`セーブデータの読み込みに失敗しました: ${e.message}`, 'err');
        }
    }

    if (susansSaveExportBtn) susansSaveExportBtn.addEventListener('click', exportSusansSaveBackup);
    if (susansSaveImportBtn && susansSaveImportFile) {
        susansSaveImportBtn.addEventListener('click', () => {
            susansSaveImportFile.value = '';
            susansSaveImportFile.click();
        });
        susansSaveImportFile.addEventListener('change', async () => {
            const file = susansSaveImportFile.files && susansSaveImportFile.files[0];
            if (!file) return;
            susansSaveImportBtn.disabled = true;
            try { await importSusansSaveBackup(file); }
            finally { susansSaveImportBtn.disabled = false; }
        });
    }

    if (susansSaveClearBtn) susansSaveClearBtn.addEventListener('click', async () => {
        if (!confirm('このブラウザに保存したSUSANSのセーブデータを消去します。\nゲームZIPそのものは変更されません。\n\n消去してよいですか？')) return;
        susansSaveClearBtn.disabled = true;
        try {
            await clearSusansDb();
            susansLastSaveSig.clear();
            for (const path of SUSANS_SAVE_PATHS) {
                const bytes = await emu.readRun(path);
                if (bytes) susansLastSaveSig.set(path, bytesSignature(bytes));
            }
            mySetStatus('ブラウザに保存したセーブデータを消去しました。現在のゲーム内データはそのままです。', 'ok');
        } catch (e) {
            mySetStatus(`セーブ消去に失敗しました: ${e.message}`, 'err');
        } finally { susansSaveClearBtn.disabled = false; }
    });

    // ---- Dante98 Reviver v0.6: games.json launcher + Dante98/Dante98II auto detection ----
    const myLauncher = document.getElementById('my-launcher');
    const myGameList = document.getElementById('my-game-list');
    const myLauncherStatus = document.getElementById('my-launcher-status');
    const myBar = document.getElementById('my-susans-bar');
    const myCurrentTitle = document.getElementById('my-current-title');
    const myHelpButton = document.getElementById('my-help-button');
    ...
    let mySelectedGame = null;

    // Public Version 0.4.1: game view -> modern SUSANS title.
    // Preserve the latest Dante98 save slots before clearing /run, then stop polling.
    async function returnSusansPublicTitle() {
        if (myTitleReturnBtn?.disabled) return;
        if (myTitleReturnBtn) myTitleReturnBtn.disabled = true;
        try {
            mySetStatus('最新のセーブを確認して、タイトルへ戻っています…');
            await persistSusansSaves(true);
            if (susansSavePollTimer) {
                clearInterval(susansSavePollTimer);
                susansSavePollTimer = null;
            }
            await closeBundle();
            myCurrentGame = null;
            ...
            mySetStatus(`タイトルへ戻れませんでした: ${e.message}`, 'err');
        } finally {
            if (myTitleReturnBtn) myTitleReturnBtn.disabled = false;
        }
    }

    if (myTitleReturnBtn) myTitleReturnBtn.addEventListener('click', returnSusansPublicTitle);
    if (myLauncherExit) myLauncherExit.addEventListener('click', exitMySusans);
    if (myHelpButton && myHelpDialog) {
        myHelpButton.addEventListener('click', () => myHelpDialog.showModal());
        myHelpClose?.addEventListener('click', () => myHelpDialog.close());
        myHelpDialog.addEventListener('click', (e) => {
            if (e.target === myHelpDialog) myHelpDialog.close();
        });
    }
    if (myTrajectoryButton && myTrajectoryDialog) {
        myTrajectoryButton.addEventListener('click', () => myTrajectoryDialog.showModal());
        myTrajectoryClose?.addEventListener('click', () => myTrajectoryDialog.close());
        myTrajectoryDialog.addEventListener('click', (e) => {
            if (e.target === myTrajectoryDialog) myTrajectoryDialog.close();
        });
    }
    document.addEventListener('keydown', (e) => {
        if (e.key !== 'Escape') return;
        if (myHelpDialog?.open) myHelpDialog.close();
        if (myTrajectoryDialog?.open) myTrajectoryDialog.close();
    });


    function myLauncherMessage(text, isError = false) {
        if (!myLauncherStatus) return;
        myLauncherStatus.textContent = text || '';
        myLauncherStatus.classList.toggle('err', !!isError);
    }

    function stopMySusansTitleMusic() {
        try {
            const symbolFrame = document.querySelector('.my-symbol-wrap iframe');
            if (symbolFrame && symbolFrame.contentWindow) {
                symbolFrame.contentWindow.postMessage({ type: 'PLIP_STOP_AUDIO' }, '*');
            }
        } catch (error) {
            console.warn('SUSANS title music stop failed:', error);
        }
    }

    async function bootMyDanteGame(game) {
        if (!game || !game.file) return;
        // START後はDante98本編のゲーム音を主役にする。
        ...
        try {
            ...
            if (!res.ok) throw new Error(`${game.file} fetch failed (${res.status})`);
            const blob = await res.blob();
            const file = new File([blob], game.file, { type: 'application/zip' });
            await openDropped(file, false);
            await restoreSusansBrowserSave();

            function findRunEntry(filename) {
                const escaped = filename.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                const rx = new RegExp('(^|/)' + escaped + '$', 'i');
                return loadedEntries.find((e) => rx.test(e.name));
            }

            // Dante98II has a simple editor entry point (ED.BAT). Dante98 uses AUTOEXEC.BAT.
            // Prefer ED.BAT when present; otherwise fall back to AUTOEXEC.BAT.
            const dante98iiEntry = findRunEntry('ED.BAT');
            const dante98Entry = findRunEntry('AUTOEXEC.BAT');
            const launchEntry = dante98iiEntry || dante98Entry;
            const detectedType = dante98iiEntry ? 'Dante98II' : (dante98Entry ? 'Dante98' : 'unknown');
            if (!launchEntry) throw new Error('Dante98 / Dante98II の起動ファイル（AUTOEXEC.BAT / ED.BAT）を判定できません');
            selectEntry(launchEntry);
            if (!selectedEntry || runButton.disabled) throw new Error(`${launchEntry.name} could not be selected`);

            myCurrentGame = game;
            if (myCurrentTitle) myCurrentTitle.textContent = `SUSANS — ${detectedType}`;
            if (myLauncher) myLauncher.hidden = true;
            if (myBar) myBar.hidden = false;
            if (touchPadEl) touchPadEl.hidden = false;
            runStatusEl.textContent = `SUSANS — ${detectedType} detected; starting ${launchEntry.name}…`;
            startSusansSavePolling();
            runButton.click();
        } catch (e) {
    ...
    async function showMyDanteLauncher() {
        ...
        if (params.get('manual') === '1') {
            if (myLauncher) myLauncher.hidden = true;
            if (myBar) myBar.hidden = false;
            if (!document.body.classList.contains('my-susans-mode')) document.body.classList.remove('panel-hidden');
            runStatusEl.textContent = 'Dante98 Reviver manual mode — use Open / Run as in QuuBee';
            return;
        }
        try {
