...
(async function main() {
    ...
    async function returnMySusansLauncher() {
        const ok = confirm('タイトルへ戻ります。\n\n編集済みZIPを書き出していない変更は失われます。\n元のゲームZIPは変更されません。\n\nタイトルへ戻ってよいですか？');
        if (!ok) return;
        if (myTitleReturnBtn) myTitleReturnBtn.disabled = true;
        try {
            mySetStatus('Dante98を停止し、タイトルへ戻っています…');
            await closeBundle();
            myCurrentGame = null;
    ...
    if (myTitleReturnBtn) myTitleReturnBtn.addEventListener('click', returnMySusansLauncher);
    if (myQbPanelBtn)   myQbPanelBtn.addEventListener('click', toggleMyQbPanel);

    // ---- MY SUSANS v0.7.11: games.json menu + direct ZIP launcher + Dante98/Dante98II auto detection ----
    ...
    const myOpenGameZip = document.getElementById('my-open-game-zip');
    const myGameZipInput = document.getElementById('my-game-zip-input');
    ...
    function stopMySusansTitleMusic() {
        try {
            ...
            const nowPlayingTrack = document.getElementById('my-now-playing-track');
            if (nowPlayingTrack) nowPlayingTrack.textContent = '停止中';
        } catch (error) {
            console.warn('MY SUSANS title music stop failed:', error);
        }
    }

    async function bootMyDanteFile(file, game) {
        if (!file) return;

        // START後はDante98側のゲーム音を主役にするため、
        // タイトル画面のBGM／主題歌を起動処理より先に必ず停止する。
        ...
        if (myOpenGameZip) myOpenGameZip.disabled = true;

        try {
            const displayName = (game && game.name) || file.name.replace(/\.zip$/i, '');
            myLauncherMessage(`${displayName} を読み込んでいます…`);
            ...
            const dante98iiGameEntry = findRunEntry('GAME.BAT');
            const launchEntry = dante98iiEntry || dante98Entry || dante98iiGameEntry;
            const detectedType = dante98iiEntry
                ? 'Dante98II (編集)'
                : (dante98Entry ? 'Dante98' : (dante98iiGameEntry ? 'Dante98II (プレイ)' : 'unknown'));

            if (!launchEntry) {
                throw new Error('Dante98 / Dante98II の起動ファイル（AUTOEXEC.BAT / ED.BAT / GAME.BAT）を判定できません');
            }

            selectEntry(launchEntry);
            if (!selectedEntry || runButton.disabled) {
                throw new Error(`${launchEntry.name} could not be selected`);
            }
            ...
            console.error('MY SUSANS boot failed:', e);
    ...
    async function bootMyDanteGame(game) {
        ...
        try {
            ...
            await bootMyDanteFile(file, game);
        } catch (e) {
            console.error('MY SUSANS registered game load failed:', e);
            myLauncherMessage(`起動できません: ${e.message}`, true);
            if (myOpenGameZip) myOpenGameZip.disabled = false;
            if (myStartButton) myStartButton.disabled = !mySelectedGame;
        }
    ...
    if (myOpenGameZip && myGameZipInput) {
        myOpenGameZip.addEventListener('click', () => {
            myGameZipInput.value = '';
            myGameZipInput.click();
        });

        myGameZipInput.addEventListener('change', async () => {
            const file = myGameZipInput.files && myGameZipInput.files[0];
            if (!file) return;
            if (!/\.zip$/i.test(file.name)) {
                myLauncherMessage('ZIPファイルを選んでください。', true);
                return;
            }
            await bootMyDanteFile(file, {
    ...
    async function showMyDanteLauncher() {
        ...
        try {
            ...
            myGameList.replaceChildren();
            for (const game of games) {
                if (!game || typeof game.file !== 'string' || !game.file.toLowerCase().endsWith('.zip')) continue;
                const btn = document.createElement('button');
                btn.type = 'button';
                btn.className = 'my-game-button';
                btn.textContent = game.name || game.file.replace(/\.zip$/i, '');
                btn.addEventListener('click', () => {
                    mySelectedGame = game;
                    myGameList.querySelectorAll('.my-game-button').forEach(item => item.classList.remove('is-selected'));
                    btn.classList.add('is-selected');
                    if (myStartButton) myStartButton.disabled = false;
                    myLauncherMessage(`${game.name || game.file} を選択しました。STARTで起動します。`);
                });
                myGameList.appendChild(btn);
            }
            if (!myGameList.children.length) throw new Error('有効なZIPがgames.jsonにありません');
            mySelectedGame = null;
            if (myStartButton) {
                myStartButton.disabled = true;
                myStartButton.onclick = () => {
                    if (mySelectedGame) bootMyDanteGame(mySelectedGame);
                };
            }
            myLauncherMessage('ゲームを選択してください。');
        } catch (e) {
