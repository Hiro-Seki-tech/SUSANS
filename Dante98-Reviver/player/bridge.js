...
(async function main() {
    ...
    const myBackTitleBtn = document.getElementById('my-back-title');
    ...
    async function backToReviverTitle() {
        const ok = confirm('タイトルに戻ります。\n\n現在のゲームをZIP保存していない変更やゲーム内セーブは失われます。\n残したい場合は「キャンセル」を押し、先に「現在のゲームをZIP保存」を行ってください。\n\nタイトルに戻ってよいですか？');
        if (!ok) return;
        if (myBackTitleBtn) myBackTitleBtn.disabled = true;
        try {
            mySetStatus('現在のゲームを閉じています…');
            await closeBundle();
            if (myBar) myBar.hidden = true;
            if (myLauncher) myLauncher.hidden = false;
            if (myOpenGameZip) myOpenGameZip.disabled = false;
            if (myGameZipInput) myGameZipInput.value = '';
            if (myCurrentTitle) myCurrentTitle.textContent = 'Dante98 Reviver';
            myLauncherMessage('Dante98 / Dante98II のゲームZIPを選んでください。');
            document.body.classList.add('panel-hidden');
            setTimeout(() => fitCanvas(offscreen.width || 640, offscreen.height || 400), 0);
        } catch (e) {
            console.error('Dante98 Reviver return-to-title failed:', e);
            mySetStatus(`タイトルへ戻れません: ${e.message}`, 'err');
        } finally {
            if (myBackTitleBtn) myBackTitleBtn.disabled = false;
        }
    }
    ...
    if (myBackTitleBtn) myBackTitleBtn.addEventListener('click', backToReviverTitle);
    ...
    // ---- Dante98 Reviver v0.9.1: direct ZIP launcher + return-to-title + Dante98/Dante98II auto detection ----
    ...
    async function bootMyDanteFile(file, game) {
        ...
        try {
            ...
            if (!launchEntry) throw new Error('Dante98 / Dante98II の起動ファイル（AUTOEXEC.BAT / ED.BAT / GAME.BAT）を判定できません');
            selectEntry(launchEntry);
            if (!selectedEntry || runButton.disabled) throw new Error(`${launchEntry.name} could not be selected`);

            myCurrentGame = { name: displayName, file: file.name };
            if (myCurrentTitle) myCurrentTitle.textContent = `Dante98 Reviver — ${displayName} [${detectedType}]`;
            if (myLauncher) myLauncher.hidden = true;
            if (myBar) myBar.hidden = false;
            runStatusEl.textContent = `Dante98 Reviver — ${detectedType} detected; starting ${launchEntry.name}…`;
    ...
    if (myOpenGameZip && myGameZipInput) {
        ...
        myGameZipInput.addEventListener('change', async () => {
            ...
            await bootMyDanteFile(file, { name: file.name.replace(/\.zip$/i, ''), file: file.name });
        });
    }
